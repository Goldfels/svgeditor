...
$.widget( ".slidein", {
	...
	_id_wrapper: "#slidein-wrapper",     // id for page content
	_wrapper_handle: "slidein-handle",   // page handle will go here
    ...
    _create: function () {
		...
		this._wrapper_handle = $(this._id_wrapper);
    ...
    open: function ( open ) {
        ...
        if ( this.options.disabled ) {  // can't open / close a disabled panel (freeze in current state)
            ...
            if ( this.options.open != open ) {
                ...
                $( this._grab_handle ).animate( prop, this.options.speed , "swing" );
                $( this._wrapper_handle ).animate( prop, this.options.speed , "swing" );

                $( this.element ).animate( prop, this.options.speed, "swing", function() {
                    self._sliding = false;
